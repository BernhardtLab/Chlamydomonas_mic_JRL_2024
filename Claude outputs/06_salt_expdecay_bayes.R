...
#
# TRAITS (all derived from the fitted curve, per draw -- closed form, no
# root-finding needed):
#   mu_max   = mmin + A                       growth at salt = 0
#   salt_tol = log(2A / (A - mmin)) / k       salt at half of mu_max  <- headline
#   s_lim    = log(-A / mmin) / k             salt where mu = 0 (lethal limit;
#                                             defined only when mmin < 0)
#
# What this script produces:
#
#   Models/06_salt_brms.rds                    the fitted, pooled brms model
#   Models/salt_individual/salt_mic_*.rds      the individual fits
#
# Data-processed:
#   06_salt_traits_bayes.csv      per-microbe trait posteriors (med + HDI)
#   06_salt_microbe_effects.csv   microbe - none trait contrasts
#
# Figures-misc:
#   06_fig_salt_fits.png          fitted curves over the data for both models
#   06_fig_salt_effects.png       microbe-vs-control effect posteriors
...
mod.dir  <- "Models"
ind.dir  <- file.path(mod.dir, "salt_individual")   # per-microbe fits go here
dir.create(ind.dir, showWarnings = FALSE, recursive = TRUE)

# inputs / outputs (single source of truth)
mu.file      <- "02_chlamy_mus.csv"                    # input: per-well mu (from 02)
pooled.file  <- file.path(mod.dir, "06_salt_brms")     # brms appends .rds itself
traits.file  <- file.path(proc.dir, "06_salt_traits_bayes.csv")
effects.file <- file.path(proc.dir, "06_salt_microbe_effects.csv")
...
salt.formula <- bf(
  ...
  k    ~ 1 + block + (1 | mic),
  nl = TRUE
) # all three parameters vary by microbe, block fixed; natural scale

## VERIFY the three intercept priors against the nls ballpark above
## (mmin ~ -0.13, A ~ 1.65, k ~ 0.31). k must stay > 0 (a negative k flips
## the decay into runaway exponential growth); its intercept prior is
## centred well away from 0. If any chain drives k negative -> divergences,
## the fix is to model k on the log scale (k = exp(logk)), the Monod trick.
salt.priors <- c(
  ...
  prior(exponential(1), class = "sigma"),                      # start constant; see diagnostics
...
# block fixed on the parameter it most affects in the pooled model.
## VERIFY with the pooled block effects, then set block on that parameter:
##   round(fixef(salt.fit)[grepl("block", rownames(fixef(salt.fit))), ], 3)
# Default below assumes A (the amplitude / height) carries the block signal.
# If mmin or k also clear the block-effect bar, add block there too (this is
# a 3-param curve, so block on 2 params is still cheap -- see the Monod fit).

f_ind <- bf(
  mu ~ mmin + A * exp(-k * salt),
  mmin ~ 1,
  A    ~ 1 + block,      # block fixed here (move/extend per the table above)
  k    ~ 1,
  nl = TRUE
)

p_ind <- c(
  prior(normal(-0.13, 0.15), nlpar = "mmin"),                  # intercept only
  prior(normal(1.65,  0.40), nlpar = "A", coef = "Intercept"),
  prior(normal(0,     0.30), nlpar = "A"),                     # block contrasts
  prior(normal(0.31,  0.15), nlpar = "k"),                     # intercept only
  prior(exponential(1), class = "sigma")
)

fit_one <- function(m) {
  d   <- droplevels(subset(df.s, mic == m))
  nbi <- ncol(model.matrix(~ 1 + block, data = d))   # block coefs for A in this microbe
  init_ind <- function() list(
    b_mmin = as.array(-0.13),
...
set.seed(1); NSUB <- Inf          # closed-form traits, so full posterior is cheap
...
# (A) trait table -> 06_salt_traits_bayes.csv
...
# (B) microbe - none contrasts -> 06_salt_microbe_effects.csv
...
ggsave(file.path(fig.dir, "06_fig_salt_fits.png"), fig.fits, width = 13, height = 9, dpi = 300)
...
fig.eff <- effects |>
  ...
  theme_bw()
ggsave(file.path(fig.dir, "06_fig_salt_effects.png"), fig.eff, width = 10, height = 6, dpi = 300)
